...
export const NewsPublicationPage: React.FC = () => {
  ...
  const fetchPub = async () => {
    try {
      const res = await newsPublicationsApi.getAll(0, 1);
      // Since there's no direct getById for news, we fetch the list
      // Actually news publication uses the same publication endpoint pattern
      // But looking at the controller, news_publications doesn't have getById
      // Let's get from the list and find, or use publications endpoint
      // We'll just store what we fetched
    } catch {
      toast.error('Ошибка загрузки публикации');
    }
  };
  ...
  const fetchComments = async (p = 0) => {
    try {
      ...
      // comments might fail if pub not found
    }
  };
  ...
  useEffect(() => {
    const loadAll = async () => {
      setLoading(true);
      try {
        // We use the general publications endpoint to get news pub by id
        // NewsPublicationController doesn't have getById, so we'll try publications endpoint
        // Since they share the same PublicationResponseDto, let's try /publications/:id
        const { publicationsApi } = await import('../app/api/publications');
        const res = await publicationsApi.getById(Number(id));
        // Map to PublicationResponse format
        setPub({
          id: res.data.id,
          title: res.data.title,
          description: res.data.description,
          created_at: res.data.created_at,
          author_nickname: res.data.author_nickname,
