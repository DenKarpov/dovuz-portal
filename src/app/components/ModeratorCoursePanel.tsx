...
import { Download, Edit2, FileText, History, Paperclip, Plus, Send, Trash2 } from 'lucide-react';
...
export const ModeratorCoursePanel: React.FC<ModeratorCoursePanelProps> = ({ courseId, mode, category = 'LESSON' }) => {
  ...
  const handleSaveLesson = async () => {
    ...
    try {
      ...
      const { deadline_local, ...rest } = lessonForm;
    ...
    <div className={mode === 'edit' ? 'w-full' : 'lg:col-span-1'}>
      <div className="bg-card rounded-2xl border border-border overflow-hidden shadow-sm">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between bg-muted/30">
          <span className="text-sm font-semibold text-foreground">Уроки</span>
          ...
          <div className="divide-y divide-border">
            {lessons
              ...
              .map((l) => {
                ...
                return (
                  <div key={l.id} className="flex items-center justify-between px-4 py-3 hover:bg-muted/40 transition-colors">
                    {mode === 'review' ? (
                      <button
      ...
      >
        <div className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-muted-foreground mb-1">
                Описание практического задания <span className="text-red-400">*</span>
              </label>
              <textarea
            ...
            <div className="rounded-xl border border-border bg-muted/30 p-4 space-y-2">
              ...
              {historyLoading ? (
                ...
                <ul className="space-y-2 max-h-40 overflow-y-auto text-xs">
                  {reviewHistory.map((h) => (
                    <li key={h.id} className="rounded-lg border border-border bg-card/80 px-2.5 py-2">
                      <div className="flex justify-between gap-2 text-muted-foreground">
                        <span>{new Date(h.created_at.replace(' ', 'T')).toLocaleString('ru')}</span>
                        <span>{h.reviewer_nickname ?? '—'}</span>
                      </div>
                      <div className="text-foreground mt-0.5 font-medium">{historyStatusRu(h.status_after)}</div>
                      {h.score != null && (
          ...
          {reviewModal?.status === 'ACCEPTED' ? (
            <div className="flex gap-3 pt-1">
              <button
                ...
                className="flex-1 py-2.5 bg-primary text-primary-foreground text-sm font-semibold rounded-xl hover:opacity-90"
                type="button"
              >
                Закрыть
              </button>
